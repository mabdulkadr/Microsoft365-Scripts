<#
.TITLE
    Get-EntraDirectoryAudit - Entra ID Directory Change Audit
...
.DESCRIPTION
    Pulls directory audit logs showing recent changes for users, groups, applications, role assignments, and Conditional Access policies with actor details and timestamps for governance review.
        ...
        Scope & safety:
        - Read-only Graph queries; never modifies directory objects.
        Degradation behavior:
        - Empty audit windows render as empty reports without error.
        Output contract:
        - Console summary plus CSV beside the script; exit 0 = success, 1 = failure.

.TAGS
    Reporting,EntraID,Audit,Graph
...
.NOTES
    - Requires Microsoft.Graph.Authentication module.
        - Read-only; no directory modifications.
        - Logs: C:\ProgramData\Get-EntraDirectoryAudit\Logs\
#>
...
$SolutionName = 'Get-EntraDirectoryAudit'
$ScriptMode   = 'run'
...
Write-Log -Message "Script started: Get-EntraDirectoryAudit" -Level 'INFO'
...
# ============================================================================
# RETRIEVE - directory audit events for the window, optionally by category.
# ============================================================================

Write-Log -Message "=== DIRECTORY AUDIT LOG (last $Hours hours) ===" -Level 'INFO'
...
Write-Log -Message "Querying audit logs..." -Level 'DEBUG'
try {
    $auditLogs = Get-MgGraphAllPages -Uri "https://graph.microsoft.com/v1.0/auditLogs/directoryAudits?`$filter=$filter&`$top=500&`$orderby=activityDateTime desc"
} catch [System.Exception] {
    Finish-Script -ExitCode 2 -Message "Failed to query directory audits: $($_.Exception.Message)" -Level 'ERROR'
}
$auditLogs = @($auditLogs)
Write-Log -Message "$($auditLogs.Count) audit events retrieved" -Level 'SUCCESS'

if ($auditLogs.Count -eq 0) {
    Write-Log -Message "No audit events found for the specified period." -Level 'WARNING'
    return
}
...
Write-Log -Message "--- Events by Category ---" -Level 'WARNING'
foreach ($cg in $catGroups) {
    Write-Log -Message "$($cg.Name) : $($cg.Count)" -Level 'INFO'
}
...
Write-Log -Message "--- Top Initiators ---" -Level 'WARNING'
foreach ($ig in $initiatorGroups) {
    Write-Log -Message "$($ig.Name) : $($ig.Count) change(s)" -Level 'INFO'
}

# Activity breakdown
$actGroups = $auditLogs | Group-Object activityDisplayName | Sort-Object Count -Descending | Select-Object -First 15
Write-Log -Message "--- Top Activities ---" -Level 'WARNING'
...
# Process events
Write-Log -Message "=== RECENT CHANGES (newest first) ===" -Level 'INFO'
...
foreach ($event in ($auditLogs | Select-Object -First 50)) {
    ...
    $tsDisplay = try { ([datetime]$timestamp).ToString('yyyy-MM-dd HH:mm') } catch [System.Exception] {
        $timestamp
    }

    Write-Log -Message "$tsDisplay [$result] $activity" -Level 'INFO'
    Write-Log -Message "By: $initiator | Target: $targetStr" -Level 'DEBUG'
    if ($changeStr -ne '-') {
        Write-Log -Message "Changes: $changeStr" -Level 'INFO'
    }
...
if ($auditLogs.Count -gt 50) {
    Write-Log -Message "... $($auditLogs.Count - 50) more events (see CSV)" -Level 'DEBUG'
}
...
if ($riskyEvents.Count -gt 0) {
    Write-Log -Message "--- Sensitive Operations ($($riskyEvents.Count)) ---" -Level 'ERROR'
    foreach ($re in ($riskyEvents | Select-Object -First 15)) {
        ...
        Write-Log -Message "$($re.activityDisplayName) by $init" -Level 'INFO'
    }
}

$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$csvPath = if ($ExportPath) { $ExportPath } else { Join-Path $scriptDirectory "DirectoryAudit_$stamp.csv" }
...
$tableRows = foreach ($rowRef in ($htmlRows | Select-Object -First 500)) {
    $resultBadge = switch ("$($rowRef.Result)") { 'success' { 'low' } 'failure' { 'critical' } default { 'medium' } }
    '<tr><td>' + [System.Net.WebUtility]::HtmlEncode("$($rowRef.Timestamp)") + '</td>' +
    '<td>' + [System.Net.WebUtility]::HtmlEncode("$($rowRef.Activity)") + '</td>' +
    '<td>' + [System.Net.WebUtility]::HtmlEncode("$($rowRef.Category)") + '</td>' +
    '<td><span class="badge ' + $resultBadge + '">' + [System.Net.WebUtility]::HtmlEncode("$($rowRef.Result)") + '</span></td>' +
    '<td><code>' + [System.Net.WebUtility]::HtmlEncode("$($rowRef.InitiatedBy)") + '</code></td>' +
    '<td>' + [System.Net.WebUtility]::HtmlEncode("$($rowRef.Targets)") + '</td></tr>'
}
$tableNote = if ($htmlRows.Count -gt 500) { "<p>Showing 500 of $($htmlRows.Count) events; full data is in the CSV.</p>" } else { '' }
$tableHtml = '<div class="section-title">Recent Changes (newest first)</div>' +
    '<div class="card"><h2>Directory Events (' + $htmlRows.Count + ')</h2>' +
    '<table><thead><tr><th>Timestamp</th><th>Activity</th><th>Category</th><th>Result</th><th>Initiated By</th><th>Targets</th></tr></thead><tbody>' +
    ($tableRows -join "`n") + '</tbody></table>' + $tableNote + '</div>'

$failCount = @($htmlRows | Where-Object { $_.Result -eq 'failure' }).Count
$riskyCount = @($riskyEvents).Count
$kpis = @(
    @{ value = "$($auditLogs.Count)"; label = "Events ($Hours hours)"; color = '' },
    @{ value = "$($catGroups.Count)"; label = 'Categories'; color = '#0f62fe' },
    @{ value = "$failCount"; label = 'Failed operations'; color = '#da1e28' },
    @{ value = "$riskyCount"; label = 'Sensitive operations'; color = '#f1c21b' }
)
$tenantId = if ($mgContext) { $mgContext.TenantId } else { '' }
Export-StandardHtmlReport -OutputPath $htmlPath -Title 'Entra Directory Audit' -Subtitle ("Events: $($auditLogs.Count) | Window: last $Hours hours | Category: $Category") `
    -Tenant $tenantId -Body $tableHtml -Kpis $kpis -Version '1.0.1' -ReportName 'Entra Directory Audit'
