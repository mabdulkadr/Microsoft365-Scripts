...
if ($All -and $UserPrincipalNames) {
    Write-Host "Parameter -All and -UserPrincipalNames cannot be used together." -ForegroundColor Yellow
    return
}
...
foreach ($excludedUser in $ExcludedNotFound) {
    Write-Host "Can't find Microsoft Entra ID account for user $excludedUser" -ForegroundColor Red
}
...
foreach ($userNotFound in $UsersNotFound) {
    Write-Host "Can't find Microsoft Entra ID account for user $userNotFound" -ForegroundColor Red
}
